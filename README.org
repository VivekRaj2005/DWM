#+TITLE: DWM Workspace
#+AUTHOR: Vivek Raj
#+DESCRIPTION: DWM Workspace Information
#+STARTUP: showeverything
#+OPTIONS: toc:3
#+CAPTION: DWM Desktop Preview
#+ATTR_ORG: :width 900
[[file:Main.png]]
...
- [[#dwm-config][DWM Config]]
  ...
  - [[#layouts][Layouts]]
  - [[#autostart-commands][Autostart commands]]
  - [[#spawn-commands][Spawn commands]]
  - [[#keybindings][Keybindings]]
- [[#images--screenshots][Images / Screenshots]]

* Overview
This workspace contains three Small Tools from suckless: a patched/packaged
version of `dwm` (dynamic window manager), `dmenu` (dynamic menu) and
`dwmblocks` (statusbar utility). The tree groups each project into its own
directory and includes local patches and helper files.

* Components
** DWM
- Location: [[file:DWM/dwm.c]]
- Main build target: See [[file:DWM/Makefile]]

** DMenu
- Location: [[file:DMenu/dmenu.c]]
- Main build target: See [[file:DMenu/Makefile]]

** DWMBlocks
- Location: [[file:DWMBlocks/dwmblocks.c]]
- Main build target: See [[file:DWMBlocks/Makefile]]
...
Notes:
- Each project uses a `config.def.h` that is copied to `config.h` by
	the Makefile; edit `config.def.h` or create a local `config.h` before
	building if you need custom configuration.

* Patches included
This repository contains a set of optional patches for `dwm` and `dmenu`.
Apply them from the respective subdirectories as desired.

- DWM patches:
	- [[file:DWM/patches/dwm-alwayscenter-20200625-f04cac6.diff]]
	- [[file:DWM/patches/dwm-hide_vacant_tags-6.4.diff]]
	- [[file:DWM/patches/dwm-rotatestack-20161021-ab9571b.diff]]
	- [[file:DWM/patches/dwm-restartsig-20180523-6.2.diff]]
	- [[file:DWM/patches/dwm-uselessgap-20211119-58414bee958f2.diff]]
	- [[file:DWM/patches/dwm-notitle-6.2.diff]]

- DMenu patches:
	- [[file:DMenu/patches/dmenu-fuzzymatch-4.6.diff]]
	- [[file:DMenu/patches/dmenu-highlight-20201211-fcdc159.diff]]
	- [[file:DMenu/patches/dmenu-dracula-20211128-d78ff08.diff]]

* Configuration
- To change defaults, edit the `config.def.h` files in each project and
	then run `make` to regenerate `config.h` and build the binaries.

* DWM Config 
This section documents the active settings and keybindings currently defined
in your `dwm` configuration.
